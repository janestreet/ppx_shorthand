...
open struct
  ...
  let%template option_value_local_exn = [%eta1 Option.value_exn [@mode local]]
  let%template option_first_some_local = [%eta2 Option.first_some [@mode local]]
  let bool_select = [%eta3 Bool.select]
end
